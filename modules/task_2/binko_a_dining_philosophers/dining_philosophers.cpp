...
#include "../../../modules/task_2/binko_a_dining_philosophers/dining_philosophers.h"

#include <cstring>
#include <iostream>

enum tags { add = 1, disconect_proc };
...
void waiter(int64_t* data, int mpi_size) {
  ...
  while (up_process != 0) {
    ...
    MPI_Recv(&delta, 1, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD,
             &status);
